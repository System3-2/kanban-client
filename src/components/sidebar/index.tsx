...
export const Sidebar = () => {
  //TODO: Recieve project data prop from parent
  ...
  return (
    <>
      ...
      >
        ...
        <Box>
          {/* sidebar links */}
          {sidebarLinks.slice(0, 2).map((items) => {
            const { icon, name, path } = items;
            //@ts-ignore
            const Icon = Icons[icon || "board"];

            return (
              <>
                <MenuItem component={Link} href={path}>
                  <MenuList
                    //@ts-ignore
                    sx={{
                      position: "relative",
                      ...
                      textDecoration: "none",
                      color: (theme) =>
                        path === pathname && theme.palette.primary.main,
                    }}
                  >
                    <Box
                      sx={{
          ...
          {/* Divider */}
          <Divider sx={{ color: (theme) => theme.palette.grey[900] }} />

          {/* sidebar links */}
          {sidebarLinks.slice(2).map((items) => {
            const { icon, name, path } = items;
            //@ts-ignore
            const Icon = Icons[icon || "board"];
...
const ProjectAvatar = ({ className, size }: AvatarProps) => (
